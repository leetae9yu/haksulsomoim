...
import type { LocalOcrPort, OcrCandidate } from "../ocr/local-ocr";
import type {
  ScreenMaskRegion,
  ScreenTextRegion,
...
export class PlaywrightSecureBrowser implements SecureBrowserPort {
  ...
  async start(url: string): Promise<void> {
    if (this.#browser !== undefined) {
      throw new Error("Secure browser is already started");
    }
    ...
    await this.#context.route("**/*", async (route) => {
      ...
      const localProtocol = requestUrl.protocol === "data:" || requestUrl.protocol === "blob:";
      if (localProtocol || this.#allowedHosts.has(requestUrl.hostname.toLowerCase())) {
  ...
  async inspect(): Promise<SecureBrowserInspection> {
    ...
    const [domCandidates, ocrResult] = await Promise.all([
      this.#readDomCandidates(page),
      this.#ocr.recognize(rawScreenshot),
    ]);
    const ocrCandidates = ocrResult.status === "readable" ? ocrResult.candidates : [];
  ...
  #deduplicate(
    candidates: readonly (ScreenTextRegion | OcrCandidate)[],
