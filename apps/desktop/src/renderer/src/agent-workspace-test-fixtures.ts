  ...
  return agentRunProjectionSchema.parse({
    ...
    steps: [
      ...
      { kind: "tool-started", stepId: "step-law", toolName: "search-official-law" },
      {
        ...
        outcome: "completed",
        dependsOnStepId: "step-inspect-done",
      },
      { kind: "tool-started", stepId: "step-draft", toolName: "write-local-draft" },
      {
        kind: "tool-finished",
        stepId: "step-draft-done",
        toolName: "write-local-draft",
        outcome: "completed",
        dependsOnStepId: "step-law-done",
  ...
  const api: DesktopApi = {
    ...
    openAgentArtifact: mock(async (request) =>
      agentArtifactViewSchema.parse({
        ...
        title: "민사 지급명령 검토 초안",
        sections: [{ heading: "마스킹된 사건 사실", text: "amount: [ACCOUNT_1]" }],
        citationIds: ["law-1"],
      }),
    ),
    startAgentRun,
