...
import { containsDirectIdentifier } from "./redaction";
...
export class SecureComputerActionGate {
  ...
  constructor(allowedHosts: readonly string[]) {
    if (allowedHosts.length === 0) throw new TypeError("At least one allowed host is required");
    this.#allowedHosts = new Set(allowedHosts.map((host) => host.toLowerCase()));
  }

  evaluate(
    input: Readonly<{ url: string; action: SecureComputerAction; target?: SecureBrowserTarget }>,
  ): SecureComputerGateDecision {
    ...
    if (input.action.kind === "scroll") return { outcome: "allowed" };
    if (input.target === undefined) return { outcome: "rejected", reason: "target-unavailable" };
