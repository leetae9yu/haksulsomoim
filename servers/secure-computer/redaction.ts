...
const IDENTIFIER_PATTERNS: readonly IdentifierPattern[] = [
  ...
  { kind: "PHONE", pattern: /(?<!\d)01[016789][ -]?\d{3,4}[ -]?\d{4}(?!\d)/g },
  {
    kind: "ADDRESS",
    pattern:
      /(?:서울(?:특별시)?|부산(?:광역시)?|대구(?:광역시)?|인천(?:광역시)?|광주(?:광역시)?|대전(?:광역시)?|울산(?:광역시)?|세종(?:특별자치시)?|경기(?:도)?|강원(?:특별자치도|도)?|충청[남북]도|전라[남북]도|경상[남북]도|제주(?:특별자치도|도)?)\s+[가-힣0-9·]+(?:시|군|구)(?:\s+[가-힣0-9·]+(?:시|군|구))?\s+[가-힣0-9·.-]+(?:대로|로|길|동|읍|면)\s+\d+(?:-\d+)?/gu,
  },
  {
    kind: "ACCOUNT",
    pattern:
...
const CONTEXTUAL_PERSON =
  /((?:성명|이름|신청인|송금인|채권자|채무자|원고|피고|sender)\s*[:=]\s*)([가-힣]{2,4})(?![가-힣])/giu;
